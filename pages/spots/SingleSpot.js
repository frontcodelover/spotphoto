...
import { doc, getDoc, updateDoc, deleteDoc, query, collection, getDocs, where, arrayUnion, arrayRemove  } from 'firebase/firestore';
// import { useParams } from 'react-router';
import { useAuth, db, auth } from "../firebase/firebase";
import { FaRegClock, FaMapMarkerAlt, FaBookmark, FaRegBookmark } from 'react-icons/fa';
...
function BookmarkAllready() {
  if (BookmarkAllreadyOk === false) {
    ...
    return (
      <>
        <button onClick={() => addToUser(currentUser, id)} className="btn-save"><FaRegBookmark /></button>
    ...
    return (
      <>
        <button onClick={() => removeToUser(currentUser, id)} className="btn-save"><FaBookmark /></button>
  ...
  const fetchUserWhoAddThisPost = async () => {
    try {
      ...
      if (infoUser.length === 0) {
        ...
        setInfoUserWhoAdd(infoUser.map(user =>
          <div className="add" key="1">Spot ajouté par : {user}</div>
          ))
        }
  ...
  return (
    ...
    <div className="body-size">
      ...
      { //Check if message failed
        (count === null)
          ? <div> <label htmlFor="heart">❤</label> {spots.nbLike} personnes ont aimé ce spot</div> 
          : <div> <label htmlFor="heart" className='redHeart'>❤</label>{count} personnes ont aimé ce spot</div> 
      }
    </div>

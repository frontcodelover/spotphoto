  ...
  useEffect(() => {
    const fetchBookmarkPost = async () => {
      try {
        ...
        const currentUserId = resUsers.filter(
          (user) => user.uid === currentUser.currentUser.uid
        );
        ...
        if (bookmarkedPost.length === 0) {
          ...
          setSpotNameBookMark(
            bookmarkedPost.map((spot) => (
              <div
                key={spot.uid ? spot.iud : "no spot"}
                className="grid-banner hover-scale-card"
              >
                <a href={`/spots/${spot.uid}`}>
                  <img
                    src={spot.imgUrl}
                    alt={`${
                      spot.inputs.name ? spot.inputs.name : "Aucun spot"
                    }`}
              ...
              <div className="badge body-size">
                ...
                <div className="">
                  <h1 className="text-2xl">
                    {user.inputs.pseudo ? user.inputs.pseudo : "Non renseigné"}
                  </h1>
                  <p className="text-lg pt-3 pb-9">
                    {" "}
                    {user.inputs.bio
                      ? user.inputs.bio
                  ...
                  <div className="infouser-detail">
                    <div className="flex align-middle">
                      <FaBirthdayCake className="pr-2 pt-2 text-xl" />{" "}
                      {user.inputs.age ? user.inputs.age : "Non renseigné"}{" "}
                      <br />
                    </div>
                    <div className="flex align-middle">
                      <FaGlobeAmericas className="pr-2 pt-2 text-xl" />{" "}
                      {user.inputs.personnalLink
                        ? user.inputs.personnalLink
                        : "Non renseigné"}{" "}
                      <br />
                    </div>
                    <div className="flex align-middle">
                      <FaMapMarkerAlt className="pr-2 pt-2 text-xl" />{" "}
                      {user.inputs.codePostal
                        ? user.inputs.codePostal
                        : "non renseigné"}{" "}
                      {user.inputs.ville ? user.inputs.ville : ""}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="page-account">
              ...
              <div className="section-home-two">
                <div className="body-size">
                  ...
                  <div className="section-post-home">{spotNameBookMark}</div>
                </div>
              </div>
